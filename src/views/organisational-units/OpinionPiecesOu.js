...
class OrganisationalUnits extends Component {
  ...
  getOus = async () => {
    try {
      ...
      const singleOu = data.find(ou => ou.name === 'Opinion Publishing');
  ...
  addDivision = async () => {
    const response = await fetch(`https://stepbot-cooltech-internal-api.herokuapp.com/ous/add/division/60e705d0d616322ba883c23d`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'auth-token': token
    ...
    return (
      <>
        {(fault) ? <h1 style={{textAlign: "center"}}>Access denied.</h1> :
          (this.ouAccess('60e705d0d616322ba883c23d', user)) ?
            (
              <CRow>
                <CCol sm="12" xl="12">
                  <CCard>
